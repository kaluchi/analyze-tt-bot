...
<#
.SYNOPSIS
    Тесты для конструктора и инициализации MediaFormatterService.
.DESCRIPTION
    Модульные тесты для проверки корректности создания экземпляра MediaFormatterService.
.NOTES
...
Describe "MediaFormatterService.Constructor Tests" {
    BeforeAll {
    ...
    Context "Constructor and initialization" {
        It "Should create an instance correctly" {
            InModuleScope -ModuleName AnalyzeTTBot {
                # Проверяем, что класс MediaFormatterService доступен в модуле
                {
                    $service = New-Object -TypeName MediaFormatterService
