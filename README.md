...
Этот Telegram-бот разработан для анализа и скачивания видео с TikTok с сохранением оригинального качества. Проект использует современную модульную архитектуру на PowerShell 7 с инверсией управления и PSFramework.
...
- Поиск по содержимому файлов с использованием Search-FileContent
...
- Инверсию контроля через контейнер зависимостей
- Строгую инкапсуляцию работы с внешними API и инструментами
...
- **curl** - встроен в Windows 10/11, для работы с API Telegram
...
- **PSFramework** — современное управление конфигурацией и логированием
...
- **Современная обработка ошибок** — использование `-ErrorAction` и `-ErrorVariable`
- **Централизованная конфигурация** — все параметры в PSFramework
- **Надежная работа с путями** — использование `Join-Path` для корректной работы с путями
- **Изолированная проверка зависимостей** — отдельный модуль для валидации компонентов
- **Корректная обработка HTML-тегов** — использование строковых литералов вместо прямых HTML-тегов
